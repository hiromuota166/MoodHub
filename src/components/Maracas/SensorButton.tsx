...
const SensorButton: React.FC<SensorButtonProps> = memo(
	function SensorButton(props) {
		...
		return (
			<button onClick={handleToggleSensor} className='p-4 shadow-boxOut rounded-3xl'>
				{colorModeMemo === "dark" && (
					<>
						{sensorOn ? (
	...
	(prevProps, nextProps) => {
		...
		return prevProps.handleToggleSensor === nextProps.handleToggleSensor && prevProps.colorMode === nextProps.colorMode && prevProps.sensorOn === nextProps.sensorOn;
	}
);
